...
export const fetchMovieDetails = async (moveId: string): Promise<MovieDetails> => {
    try{
        const response = await fetch(`${TMDB_CONFIG.BASE_URL}/movie/${moveId}?api_key=${TMDB_CONFIG.API_KEY}`, {
            method: 'GET',
            headers: TMDB_CONFIG.headers
        });
        ...
        if(!response.ok){
            // @ts-ignore
            throw new Error('Failed to fetch movie details:', repsonse.statusText);   
        }
